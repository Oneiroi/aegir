...
| ID | Priority | Finding | Status | Target |
|----|----------|---------|--------|--------|
| F1 | CRITICAL | Compliance patterns recompiled per call | **FIXED** | M005 post-patch |
| F2 | HIGH | SSRF DNS rebinding bypass | In progress | M006 |
| F3 | MEDIUM | Rate limiter unbounded memory growth | In progress | M006 |
| F4 | MEDIUM | Rate limiting by IP, not user identity | In progress | M006 |
| F5 | MEDIUM | Entropy threshold globally fixed, no per-context tuning | Deferred | M007 |
| F6 | LOW | MFA field parsed but not enforced | Deferred | M007 |
| F7 | LOW | Pattern updates require restart (no hot-reload) | Deferred | M008 |
| F8 | ARCH | ~62% ATLAS ceiling on pattern-based detection | Acknowledged | M009+ |
| F9 | ARCH | Crescendo attacks (AML.T0054.007) undetectable at transport layer | Acknowledged | M009+ |
| F10 | DOCS | No machine-readable scope boundary statement | Deferred | M006 |
