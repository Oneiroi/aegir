...
func newMockProxy(mock *MockSessionAnalyzer) *MCPProxy {
	...
	return NewMCPProxy(defaultCfg, logger, sanitizerMgr, complianceMgr, upstreamMgr, mock, nil, nil)
}
...
func TestTwoTierThreatResponseEdgeCases(t *testing.T) {
	...
	t.Run("Empty Session Analyzer - Should Process Normally", func(t *testing.T) {
		...
		proxy := NewMCPProxy(defaultCfg, logger, sanitizerMgr, complianceMgr, upstreamMgr, nil, nil, nil)
