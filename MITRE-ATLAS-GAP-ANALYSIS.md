...
### M006 — Activate Dead Defences *(2-3 days)*

**Goal:** Make existing code actually execute. No new detection logic required.

**Techniques addressed:** AML.T0054.001–.006, AML.T0051.001–.002, AML.T0056, AML.T0053, POLY.003–.008, SSRF.001–.004, TMP.INJ.002–.004

**Deliverables:**
1. `sanitizer/manager.go:initializePatterns()` — compile all patterns at startup into Manager fields
2. `sanitizer/manager.go:detectPromptInjection()` — iterate `GetIOCPatterns()` alongside local patterns
3. `server/mcp_proxy.go:188-192` — add configurable anomaly score block threshold
4. `server/mcp_proxy.go:307` — add `ScanForCompliance()` on upstream response
5. `server/mcp_proxy.go:handleToolsCall()` — call `validateResourceURI()` on URL-typed tool arguments
6. `auth/manager.go:createDefaultUsers()` — remove hardcoded "admin123"

**ISC gate:** `go test ./...` exits 0; integration test confirms AML.T0054.003 (roleplay), SSRF.001 (AWS metadata URL in tool arg), and PII response leakage are all blocked.

---

### M007 — Authentication Hardening + Model Extraction Resistance *(4-6 days)*

**Goal:** Close the authentication gaps and make inference-based attacks economically infeasible.

**Techniques addressed:** AML.T0012 (MFA), AML.T0005, AML.T0006

**Deliverables:**
1. `auth/manager.go:Login()` — implement TOTP validation (`github.com/pquerna/otp/totp`)
2. Rate limiter refactored to key on authenticated user identity (not just IP)
3. Cross-session repetition counter: sliding window tracking uniform query patterns per user
4. Anomaly score aggregated per session with session-level block threshold (not just per-message)
5. `detectSecrets()` extended with missing token types (Azure SAS, GCP SA JSON, Slack tokens)

**ISC gate:** MFA end-to-end test passes; rate limit test confirms per-user keying; model extraction simulation (100 uniform queries from 10 IPs) triggers session-level block.

---

### M008 — Response Integrity + Indirect Injection *(5-7 days)*

**Goal:** Treat model output and tool results as untrusted data, not pass-through.

**Techniques addressed:** AML.T0051.001 (indirect injection), AML.T0056 (full prompt extraction), AML.T0057 (full response leakage coverage)

**Deliverables:**
1. Response path: full prompt injection detection suite applied to tool call results before forwarding to model context
2. `manager.go:detectPromptInjection()` — add pure extraction patterns (no override verb required)
3. Response compliance scan coverage report: log all PII/PHI/PCI detections in model output with severity
4. Policy mode for response: configurable block vs. redact vs. log-only per data type
5. Leet-speak normalisation pass before pattern matching (closes POLY.003)
6. Base64 decode-then-scan for encoded payloads in request content (closes AML.T0051 base64 variant)

**ISC gate:** Integration test: inject PII via indirect tool result → confirm response is redacted; inject leet-speak prompt injection → confirm detection; base64-encoded `ignore all previous instructions` → confirm detection.
...
**Active enforcement rate: 19% (8/42)**
**With dead code activated: 62% (26/42)**
**Remaining genuine gaps after M006-M008: ~15%** (P3 items requiring semantic capabilities)

---

*Analysis based on HEAD `2c2f056`. Re-run against each milestone branch to track coverage improvement.*
