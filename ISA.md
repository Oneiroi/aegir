...
> **M015 UPDATE (2026-07-14, superseded same day — now fully committed + tagged).** M015 (MCP
> 2026-07-28 spec hardening — six-vector gap analysis in
> `docs/MCP-2026-07-28-SPEC-GAP-ANALYSIS.md`, plan in `docs/M015-IMPLEMENTATION-PLAN.md`) is
> **implemented, probe-green, and committed in full**: ISC-166–178 (13 ISCs). ISC-166/167/168
> (V3 desync guard + V2 `_meta` inspector, V6 task-registry scaffold) landed at `23e5af1`;
> ISC-169–178 (V4 header egress scan, V5 XSS/App-content hardening, the ISC-171/172
> task-lifecycle wiring that makes the V6 quota check actually fire, spec-transition
> replay-cache re-anchor + mandatory PKCE, V1 signed client-state verification) landed at
> `d3539e3`. Progress `165/167` → `178/180` (178 done; 2 remain open — see below, both
> pre-existing and unrelated to M015). **Tagged `v0.3.0`** on `public` — deliberately pre-1.0,
> zero community feedback yet. A follow-up docs-only commit (`236636a`) also corrected two stale
> README ATLAS-coverage claims found during this same reconciliation pass (AML.T0051.001 and
> AML.T0054.007 were both still marked "planned" long after ISC-22/ISC-52 shipped).
>
> **A real, verified gap was found and closed mid-session, not assumed from the plan.** The M015
> plan's ISC-177 ("replay protection re-anchored on identity, since Mcp-Session-Id is removed")
> presupposed replay protection was live. A repo-wide grep found `protocolguard.ReplayCache` /
> `CheckReplay` / `NewReplayCache` were **never called anywhere outside `protocolguard.go` itself**
> — ISC-118 (2026-06-05, "DONE") was tested in isolation but never wired into `HandleMCPRequest`.
> Same pattern, independently confirmed, for the ISC-171/172 task registry inherited from the
> committed `23e5af1` scaffold: `StartTask`/`StartTaskWithUserContext` were never called from the
> real request path, so the quota check it gated was permanently a no-op reading a registry
> nothing populated. Both are now actually wired (see Changelog entry below) — this ISA now
> reflects what runs, not what compiles in isolation.
>
...
- [DEFERRED-VERIFY] ISC-86: Dashboard accessible and functional in browser — probe: `https://localhost:8443/dashboard` loads, login works — **FUNCTIONALLY VERIFIED 2026-07-04, browser-render gate deferred:** with the demo server running (`:18443`), `GET /dashboard` returns 200 serving a real `<title>Aegir</title>` login page with username/password form; `POST /auth/login` with `admin` + the startup-log password returns **200 + `access_token`**; demo-flow confirms `/dashboard/stats` + `/dashboard/health` → 200. The remaining gate — the JS-rendered authenticated dashboard in a real browser — requires the Interceptor extension, which is **not installed on this machine** (`interceptor` binary absent). **Follow-up (FUP-ISC-86):** run `interceptor open https://localhost:8443/dashboard` + login on a machine with Interceptor (David's), or accept the functional proof. Cannot be `[x]` until the browser probe runs (Algorithm `[DEFERRED-VERIFY]` rule).
...
### M015 — MCP 2026-07-28 Spec Hardening

Source: `docs/MCP-2026-07-28-SPEC-GAP-ANALYSIS.md` (six-vector gap analysis) → `docs/M015-IMPLEMENTATION-PLAN.md` (ISC assignment, Fable-tier FablePlan). All 13 ISCs below implemented and probe-green at this reconciliation; commit status noted per-ISC (see M015 UPDATE note above the Status Summary).

- [x] ISC-166: [V3 — protocol confusion] When `Mcp-Method`/`Mcp-Name` HTTP headers are present, the gateway asserts they equal the JSON-RPC body `method`/tool name; mismatch is rejected -32600 before forwarding — probe: `TestHeaderBodyMethodMismatchRejected`/`TestHeaderBodyMatchAllowed`/`TestHeaderBodyNameMismatchRejected`/`TestHeaderBodyNameMatchAllowed` (`internal/protocolguard`) — **DONE 2026-07-13** (`protocolguard.ValidateWithHeaders`, wired in `HandleMCPRequest` ahead of all other checks; body reconstructed via `json.Marshal` rather than string formatting so string/null JSON-RPC `id` values don't false-block — **committed `23e5af1`**)
- [x] ISC-167: [V2 — `_meta` manipulation] Inbound `params._meta` is key-allowlisted; unknown keys are stripped or the request rejected (config-selectable) — probe: `TestMetaUnknownKeysStripped`/`TestMetaUnknownKeysRejected`/`TestMetaTraceIDAllowed`/`TestNoMetaAllowed`/`TestEmptyMetaAllowed` (`internal/meta`) — **DONE 2026-07-13** (`meta.Inspector`, wired in `HandleMCPRequest` before session analysis — **committed `23e5af1`**)
- [x] ISC-168: [V2 — `_meta` manipulation] No client-supplied `_meta` value can influence Aegir's routing or authorization decision — probe: `TestMetaTenantCannotEscalate`/`TestMultipleProhibitedKeys` (`internal/meta`) — **DONE 2026-07-13** (`Inspector.IsAuthorized` rejects `tenant`/`role`/`permission`/`scope`/`permissions`/`groups`/`teams` keys outright, independent of the allowlist path — **committed `23e5af1`**)
- [x] ISC-169: [V4 — `x-mcp-header` leakage] The secret/PII detectors (sanitizer `detectSecrets` + compliance PII/PHI/PCI) run over request **and** response HTTP headers, not only bodies — probe: `TestScanHeadersDetectsXMcpHeaderCredential`/`TestScanHeadersSkipsAuthorizationHeader`/`TestScanHeadersCleanHeaderAllowed`/`TestSecretInResponseHeaderFlagged`/`TestXMcpHeaderCredentialBlockedOverHTTP` (`internal/server`) — **DONE 2026-07-14** (new `internal/server/header_scan.go`: `scanHeaders` runs both detectors per header value, skip-listing legitimate high-entropy headers — `Authorization`/`Cookie`/`Set-Cookie`/protocol plumbing — so the auth channel itself is never flagged; wired both on the request path (before the desync guard) and the response path (after a successful upstream forward). Response-header capture required threading `upstream.MCPResponse.Headers` from the real `http.Response` in `forwardRequest` — previously discarded entirely. **committed `d3539e3`.**)
- [x] ISC-170: [V4 — `x-mcp-header` leakage] A value mapped via an `x-mcp-header` directive is specifically detected and reported as credential-in-header leakage, regardless of the underlying detector's own severity ranking — probe: `TestScanHeadersDetectsXMcpHeaderCredential` — **DONE 2026-07-14** (`headerFinding.ViaXMCPHeader` set for any header whose name has the `x-mcp-header` prefix; `headersBlocked` treats that flag as blocking on its own, independent of severity. **committed `d3539e3`.**)
- [x] ISC-171: [V6 — async DoS] Async/long-running tasks are tracked per authenticated identity with a configurable concurrency cap; exceeding it is rejected before the task is spawned — probe: `TestAsyncTaskQuotaEnforced`/`TestTotalQuotaEnforced` (`internal/tasks`) + `TestAsyncTaskQuotaEnforcedOverHTTP`/`TestAsyncTaskQuotaDormantByDefault` (`internal/server`, drives the real `HandleMCPRequest` entry point) — **DONE 2026-07-14** (`tasks.Registry` scaffold + the `AsyncTaskQuota.Enabled` pre-check were committed at `23e5af1`, but nothing called `StartTask` from the real request path — the registry the pre-check read was permanently empty, a "present but not wired" gap AGENTS.md warns against by name. Closed this session: `tools/call` (the only method that spawns real upstream work) now registers via `StartTaskWithUserContext` before forwarding, with `defer MarkFinished`; `newTaskRegistry` derives its caps from config (unlimited when `AsyncTaskQuota.Enabled` is false, so the dormant-by-default posture holds even though tracking itself always runs). **committed `d3539e3`.**)
- [x] ISC-172: [V6 — async DoS] A long-running task is cancelled server-side when the initiating client disconnects, rather than running to completion unconsumed — probe: `TestTaskCancelledOnClientDisconnect` (`internal/tasks`, registry-level context propagation) — **DONE 2026-07-14** (the task-scoped context returned by `StartTaskWithUserContext` is derived from `c.Request.Context()` and is what's now actually passed to `upstreamManager.ForwardRequest` for `tools/call` — a client disconnect cancels the same context the registry tracks, so registry bookkeeping and real cancellation are the same mechanism, not two that could drift. **committed `d3539e3`.**)
- [x] ISC-173: [V5 — stored XSS in MCP Apps] The active-content filter strips/flags event-handler attributes, `svg`-embedded script (incl. unclosed tags), and `data:` URIs to a renderable/executable MIME type, not only `<script>`/`javascript:` — probe: `TestToolResultStripsEventHandlers`/`TestToolResultStripsDataURI`/`TestToolResultStripsSvgScript`/`TestToolResultStripsEncodedScript` (`internal/server`, drives `scanToolResultForInjection`) — **DONE 2026-07-14** (`internal/sanitizer/manager.go` `sanitizeXSS`: added `xss_svg_script` — svg wrapping a script element even with no closing tag; `xss_unclosed_script` — a bare `<script...>` with no matching close; `xss_data_uri` — `data:` to `text/html`/`text/javascript`/`application/javascript`/`image/svg+xml`. New `detectEncodedXSS` decodes base64 blobs found by the existing `base64Re` scanner and re-checks the decoded bytes against the same active-content markers, closing the "encoded variants" bypass the gap analysis named. `xss_attempt` dashboard bucketing widened from an exact match to a `xss_`-prefix match (`mcp_proxy.go`) so the new subtypes still land in `dashboard.AttackXSS`. **committed `d3539e3`.**)
- [x] ISC-174: [V5 — stored XSS in MCP Apps] MCP App payloads are handled as a distinct, stricter-scanned content class, not conflated with plain tool-result text — probe: `TestMcpAppContentScannedStrict` (`internal/server`) — **DONE 2026-07-14** (new `sanitizer.Manager.SanitizeAppContent`: runs the XSS filter unconditionally — bypassing the `Sanitization.XSSPrevention` operator toggle plain-text scanning obeys, since an App panel renders its content directly — and blocks on *any* active-content detection rather than the risk-level aggregation plain text uses. Still gated behind the master `Detection.Enabled` switch (ISC-132), which an operator disabling all detection has deliberately chosen. New `extractAppContent` in `mcp_proxy.go` recognizes an explicit `"html"` field or a `resource`/`ui`/`app`-typed block with an HTML mimeType — the MCP Apps content-block wire shape is not pinned down by a schema available in this repo, so this is a documented best-effort recognition, not a verified spec citation. **committed `d3539e3`.**)
- [x] ISC-175: [V1 — cross-agent workflow hijacking] Any resumable state or task ID the gateway issues is cryptographically signed (HMAC); client-supplied state that fails verification is rejected outright, never resumed — probe: `TestUnsignedResumeStateRejected`/`TestTamperedTaskIdRejected` (`internal/statesign`, signer-level) + same-named probes in `internal/server` driving the real `tasks/status` HTTP entry point — **DONE 2026-07-14** (new `internal/statesign` package: `Signer.Sign`/`Verify` — `base64url(json(claims)) + "." + base64url(hmac-sha256(payload))`, constant-time signature comparison via `crypto/subtle`. This is genuinely net-new — Aegir had no async-task lifecycle a client could resume at all before this session (confirmed by grep: no `tasks/*` method existed in the dispatch switch). Added a minimal `tasks/status` MCP method, handled entirely locally (never forwarded — no upstream could ever resolve an Aegir-signed token) via `handleTaskStatus`; a `tools/call` response now carries the signed token in an `X-Aegir-Task-Token` header. **committed `d3539e3`.**)
- [x] ISC-176: [V1 — cross-agent workflow hijacking] Resuming a workflow enforces tenant/identity isolation — a state object issued to identity A cannot be resumed by identity B — probe: `TestCrossTenantResumeDenied` (`internal/statesign`, signer-level) + `TestCrossTenantResumeDenied` (`internal/server`, two-identity HTTP round trip: identity "alice" gets a token from her own `tools/call`, identity "bob" is denied redeeming it with 403, alice redeeming her own token still succeeds) — **DONE 2026-07-14** (`Signer.VerifyForIdentity` checks signature validity AND `claims.Identity == requestingIdentity`; `handleTaskStatus` calls this, never the identity-blind `Verify`, so an attacker with a stolen-but-validly-signed token from a different identity is still denied. **committed `d3539e3`.**)
- [x] ISC-177: [Spec-transition] Replay protection functions without `Mcp-Session-Id` — the replay cache is re-anchored on per-connection identity rather than the removed session header — probe: `TestReplayCacheWorksWithoutSessionId`/`TestReplayProtectionDisabledByDefault` (`internal/server`, real HTTP entry point, request carries no session header at all) — **DONE 2026-07-14** (discovered mid-session that `protocolguard.ReplayCache`/`CheckReplay` — credited "DONE" for ISC-118 on 2026-06-05 — was never called from production code anywhere outside `protocolguard.go` itself; this ISC both re-anchors AND, for the first time, actually wires replay detection into `HandleMCPRequest`, keyed on `userID` + stringified JSON-RPC `id`. New `Security.ReplayProtection` config, disabled by default (dormant, matching the M015 staging convention — enabling it does not change behavior for any existing caller). **committed `d3539e3`.**)
- [x] ISC-178: [Spec-transition] OAuth 2.1 + PKCE (S256) is enforced, not optional, on the auth path — probe: `TestPKCEEnforced` (`internal/auth`, 6 subtests: missing `code_challenge` rejected, `code_challenge_method=plain` rejected, missing `code_verifier` rejected, mismatched `code_verifier` rejected, correct `code_verifier` succeeds, state is single-use even after a failed PKCE attempt) — **DONE 2026-07-14** (`internal/auth/manager.go`: `OAuthLogin` now requires `code_challenge` + `code_challenge_method=S256`, storing both in `oauthStates` alongside the CSRF state (type changed from bare `time.Time` to `oauthStateEntry`); `OAuthCallback` requires `code_verifier` and constant-time-compares its SHA-256 against the stored challenge via `verifyPKCE`, rejecting before ever reaching user lookup/JWT issuance. Existing `TestOAuthLoginRedirects` updated to supply a valid PKCE challenge — the prior "just a provider param" test was itself testing behavior this ISC makes deliberately stricter. **committed `d3539e3`.**)

### M016 — LLMVault Reference Integration & Defensive Architecture Positioning

Source: Aegir as the reference defensive architecture for OWASP Top 10 for LLM Applications (2025) attack patterns taught via [LLMVault](https://github.com/CyberSunil/LLMVault) (deliberately-vulnerable CTF-style training platform, 25 labs across 3 tiers). Goal: practitioners learn attacks in LLMVault labs, then use Aegir to learn how to defend. Documentation-driven milestone; no new code ISCs (all defenses already committed in M006–M015). Deliverables: LLMVault reference in README, category-by-category defensive-mapping guide showing how Aegir ISCs cover each OWASP LLM Top 10 item.

- [x] ISC-179: [M016] LLMVault referenced in README as recommended attack-learning platform; Aegir positioned as reference defensive architecture — probe: grep -q 'LLMVault' README.md — **DONE 2026-07-16** (README updated; "Learning & validation" section added with LLMVault link and positioning statement)
- [x] ISC-180: [M016] Defensive mapping guide created: `docs/LLMVAULT-DEFENSIVE-MAPPING.md` maps each OWASP LLM Top 10 category (LLM01–LLM10) to owning Aegir ISCs, config examples, and probe tests — probe: file exists with all 10 categories documented (LLM08/LLM09 honestly marked out-of-scope) — **DONE 2026-07-16** (`docs/LLMVAULT-DEFENSIVE-MAPPING.md` created with full category-by-category defensive mapping, deployment checklist, config examples for each category; LLM08 (embeddings) and LLM09 (misinformation) marked explicitly out-of-scope with rationale)
...
| m015-desync-guard | `protocolguard.ValidateWithHeaders` asserts Mcp-Method/Mcp-Name headers match the JSON-RPC body | ISC-166 | none | true | Sonnet — DONE, committed `23e5af1` |
| m015-meta-inspector | `internal/meta.Inspector` key-allowlists `params._meta`, blocks authz-relevant keys | ISC-167, ISC-168 | none | true | Sonnet — DONE, committed `23e5af1` |
| m015-header-egress-scan | Secret/PII detectors extended to request + response HTTP headers; `x-mcp-header` flagged distinctly | ISC-169, ISC-170 | none | true | Sonnet — DONE, committed `d3539e3` |
| m015-async-task-quota | Task registry populated by a real `tools/call` lifecycle (was scaffold-only); per-identity concurrency cap + disconnect-cancel | ISC-171, ISC-172 | none | false | Sonnet — DONE, committed `d3539e3` |
| m015-app-content-hardening | `sanitizeXSS` broadened (svg/unclosed-script/data-URI/base64); `SanitizeAppContent` as a distinct stricter content class | ISC-173, ISC-174 | none | true | Sonnet — DONE, committed `d3539e3` |
| m015-spec-transition | Replay cache re-anchored on identity + actually wired; OAuth 2.1 PKCE (S256) mandatory on login+callback | ISC-177, ISC-178 | none | false | Sonnet — DONE, committed `d3539e3` |
| m015-state-verification | `internal/statesign` HMAC-signed task tokens; new `tasks/status` method enforces identity isolation on resume | ISC-175, ISC-176 | m015-async-task-quota | false | Sonnet — DONE, committed `d3539e3` |
...
- 2026-07-14: M015 (MCP 2026-07-28 spec hardening) implemented end-to-end against `docs/M015-IMPLEMENTATION-PLAN.md`'s stated sequencing (V3 → V2 → V4 → V6 → V5 → V1, spec-transition alongside V1). Session started from a mixed base: ISC-166/167/168 plus an ISC-171/172 *scaffold* (registry existed, quota pre-check existed, nothing populated the registry) were already committed at `23e5af1` by a prior pass. Rather than re-deriving that work, it was verified against the plan's named probes and built on directly. Two real, previously-uncredited gaps were found by grep (not assumed) and closed rather than left for a future session: (1) the ISC-171/172 registry was never populated — `StartTask` had no caller; (2) `protocolguard.ReplayCache` (credited "DONE" for ISC-118 back on 2026-06-05) was never called from `HandleMCPRequest` at all — both are exactly the "present ≠ wired ≠ done" failure mode AGENTS.md warns local models about, found here in code the ISA had already marked done. V1 (client-state verification) required a genuinely net-new capability — Aegir had no resumable-task concept whatsoever (`tasks/*` was not in the method dispatch switch) — so a minimal `tasks/status` method was added specifically to give the HMAC-signing work (`internal/statesign`) something real to defend, rather than landing an unused signer. All new/optional enforcement (`AsyncTaskQuota`, `ReplayProtection`) defaults to disabled/unlimited, matching the existing M015-scaffold convention — no default-on behavior change for any existing deployment. Commits are split: ISC-166–168 at `23e5af1`; ISC-169–178 uncommitted at reconciliation time because GPG commit signing could not complete an interactive pinentry from this session's sandboxed shell — David is committing that batch from his own terminal (same path used for the `23e5af1` commit). Full `go build ./... && go test ./...` green (26 packages, incl. new `internal/statesign`) at every step; no ISC above credited without its named probe passing.

- 2026-07-18: **LLMVault/SPIKEE global block-all latch fixed.** Root cause: `internal/server/mcp_proxy.go:getSessionID` ignored the `X-Session-ID` header sent by the SPIKEE target and fell back to `userID_clientIP`, collapsing all replay traffic into a single `SessionContext`. The per-session anomaly EWMA (`SessionContext.AnomalyEWMA`) ratcheted up with each adversarial detection and never decayed; once it crossed `AnomalyBlockThreshold` (default 0.70), every subsequent request from that user/IP was blocked with `HTTP 403 "Risk level: critical (critical)"`. Fix: (1) `getSessionID` now honours `X-Session-ID` (length-bounded and sanitized) before falling back; (2) `ConversationalThreatAnalyzer` applies time-based EWMA decay with a configurable half-life (`SessionAnalysis.AnomalyEWMADecayHalfLife`, default 5 minutes); (3) added `DELETE /api/security/sessions` auth-gated endpoint to clear all session state for long-running test harnesses; (4) `redteam/aegir/replay.py` gained `--recycle-after-detections N` / `--state-reset-url` to use that endpoint; (5) regression probes: `TestSessionIsolationAfterAdversarialBurst` (server) and `TestAnomalyEWMADecay` (session). Clean replay numbers (state recycled every 25 detections): adversarial block rate **62.13%** (1690/2720), benign false-positive rate **13.33%** (2/15), latency p99 **~13 ms**. The prior 94.78% block / 100% benign-FP figures were contaminated by the latch and must not be quoted.
...
- 2026-07-14 | conjectured: ISC-118 (replay protection) was a live, functioning security control — the ISA had credited it "DONE" on 2026-06-05, and the M015 plan's ISC-177 treated re-anchoring it (away from the soon-to-be-removed `Mcp-Session-Id`) as the open question, not whether it ran at all.
  refuted_by: `grep -rln "CheckReplay\|NewReplayCache" --include="*.go" .` outside `internal/protocolguard/protocolguard.go` itself returned nothing — no server code anywhere constructed a `ReplayCache` or called `CheckReplay`. Independently, the ISC-171/172 `tasks.Registry` inherited from the `23e5af1` scaffold showed the identical pattern: `StartTask`/`StartTaskWithUserContext` had no caller, so the `AsyncTaskQuota.Enabled` pre-check it gated was reading a registry that could never contain a task.
  learned: a component being exported, unit-tested, and referenced in a prior "DONE" ISA entry is not evidence it runs in production — only a grep for its call sites from the real request-handling entry point is. This is the same class of gap AGENTS.md's "present ≠ wired ≠ done" line exists to catch in *new* code; it is at least as likely to hide in code the ISA already marked done, since a passing unit test gives every appearance of completion without ever proving reachability.
  criterion_now: ISC-177 now requires (and its probe drives) replay detection through the real `HandleMCPRequest` HTTP entry point, not `protocolguard.ReplayCache` in isolation; ISC-171/172 likewise require a probe that exercises `tools/call` over real HTTP, not the registry alone.
...
**Confirmed (2026-07-14, M015 — HEAD `23e5af1` + working tree):**

- Whole-repo gate run unsandboxed at every step of this session, never substituted with a per-package or `go vet`-only check: `go build ./...` exit 0, `go test ./...` — 639 tests passed across 26 packages (new: `internal/statesign`), zero FAIL, immediately before this reconciliation.
- ISC-166: `go test ./internal/protocolguard/... -run TestHeaderBody` — 4/4 PASS (mismatch-rejected, match-allowed, name-mismatch, name-match)
- ISC-167/168: `go test ./internal/meta/...` — 7/7 PASS, incl. `TestMetaTenantCannotEscalate`/`TestMultipleProhibitedKeys`
- ISC-169/170: `go test ./internal/server/... -run 'TestScanHeaders|TestSecretInResponseHeaderFlagged|TestXMcpHeaderCredentialBlockedOverHTTP'` — 5/5 PASS
- ISC-171/172: `go test ./internal/server/... -run TestAsyncTaskQuota` — 2/2 PASS, both driving the real `HandleMCPRequest` HTTP entry point (not the registry in isolation); `go test ./internal/tasks/...` — registry-level probes still green
- ISC-173/174: `go test ./internal/server/... -run 'TestToolResultStrips|TestMcpAppContent'` — 5/5 PASS
- ISC-175/176: `go test ./internal/statesign/...` — 4/4 PASS; `go test ./internal/server/... -run 'TestUnsignedResumeStateRejected|TestTamperedTaskIdRejected|TestCrossTenantResumeDenied'` — 3/3 PASS, `TestCrossTenantResumeDenied` driving a real two-identity HTTP round trip (alice issues, bob denied 403, alice redeems 200)
- ISC-177: `go test ./internal/server/... -run TestReplay` — 2/2 PASS, `TestReplayCacheWorksWithoutSessionId` sends no session header at all
- ISC-178: `go test ./internal/auth/... -run TestPKCE` — 6/6 PASS; pre-existing `TestOAuthLoginRedirects`/`TestOAuthLoginRedirects_MissingProvider` re-verified still green after the mandatory-PKCE change (the former needed updating to supply a challenge — confirmed intentional, not a regression)
- Fingerprint scan (AGENTS.md local-model failure list) run over every file touched this session: `\!=`, escaped quotes/backticks, split identifiers, duplicate type declarations, invalid recursive types — zero matches
- `grep -nE '/Users/|sk-|AKIA|xox\[bap\]'` over changed Go files: pattern is noisy on ordinary English (`risk-level`, `task-status` etc. contain literal `sk-`) — every actual hit manually checked: the deliberately-fake test fixture `AKIAABCDEFGHIJKLMNOP`, the pre-existing `aws_access_key`/`slack_token` pattern literals in the sanitizer's own detector definitions, and `X-Aegir-Task-Token`/`TaskID`/`StateSigningKey` identifiers. No real credential and no hardcoded `/Users/`-style path in any touched file.
