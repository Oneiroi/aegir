...
> **M015 UPDATE (2026-07-14).** M015 (MCP 2026-07-28 spec hardening — the six-vector gap
> analysis in `docs/MCP-2026-07-28-SPEC-GAP-ANALYSIS.md`, plan in
> `docs/M015-IMPLEMENTATION-PLAN.md`) is **implemented and probe-green in full**: ISC-166–178
> (13 ISCs) all pass `go build ./... && go test ./...` at the point of this reconciliation.
> **Commit status is split, and that split matters:** ISC-166/167/168 (V3 desync guard + V2
> `_meta` inspector, plus the V6 task-registry scaffold) are **committed** at `23e5af1`. ISC-169
> through ISC-178 (V4 header egress scan, V5 XSS/App-content hardening, the ISC-171/172
> task-lifecycle wiring that makes the V6 quota check actually fire, spec-transition
> replay-cache re-anchor + mandatory PKCE, and V1 signed client-state verification) are
> **working-tree, not yet committed** — David is committing this batch himself in a separate
> terminal (GPG signing in this harness's sandbox can't complete an interactive pinentry). Marked
> `[ ]` `WORKING-TREE` below per the HANDOFF PROTOCOL's "uncommitted ≠ done" rule; flip to `[x]`
> once committed with the real hash. Progress `165/167` → `168/180` (168 done: the 165 prior +
> ISC-166/167/168; 12 remain `WORKING-TREE`).
>
...
| m015-header-egress-scan | Secret/PII detectors extended to request + response HTTP headers; `x-mcp-header` flagged distinctly | ISC-169, ISC-170 | none | true | Sonnet — DONE, working-tree |
| m015-async-task-quota | Task registry populated by a real `tools/call` lifecycle (was scaffold-only); per-identity concurrency cap + disconnect-cancel | ISC-171, ISC-172 | none | false | Sonnet — DONE, working-tree |
| m015-app-content-hardening | `sanitizeXSS` broadened (svg/unclosed-script/data-URI/base64); `SanitizeAppContent` as a distinct stricter content class | ISC-173, ISC-174 | none | true | Sonnet — DONE, working-tree |
| m015-spec-transition | Replay cache re-anchored on identity + actually wired; OAuth 2.1 PKCE (S256) mandatory on login+callback | ISC-177, ISC-178 | none | false | Sonnet — DONE, working-tree |
| m015-state-verification | `internal/statesign` HMAC-signed task tokens; new `tasks/status` method enforces identity isolation on resume | ISC-175, ISC-176 | m015-async-task-quota | false | Sonnet — DONE, working-tree |
